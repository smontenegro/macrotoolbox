...
% Based on the code by Aguiar & Gipinath, website:
...
[A,G] = gridmake(a',g');                    % the order of states follows A&G code
...
j = find(cdefault<=0);
fdefault = (cdefault.^(1-mu))/(1-mu); fdefault(j) = NaN;

% 6. Comupte new discount factor to account for the detrend form

beta1 = (G.^(1-mu))*beta;
beta1 = repmat(beta1,1,m);
betabad = beta1;

% 7. Solve the system of Bellman eq

[vbad,xbad,vgood,xgood,default] = solvedpos(f,fdefault,prob,beta1,betabad,lambda,azero);
